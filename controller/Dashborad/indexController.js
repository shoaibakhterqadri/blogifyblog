...
module.exports.blockUnblockSubAdmin = async (req, res) => {
  const { subAdminId } = req.params;
  const { status } = req.body;

  try {
    await userModel.findByIdAndUpdate(subAdminId, { status });
    res.status(200).json({ message: `Sub-admin ${status}ed successfully` });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      errorMessage: {
